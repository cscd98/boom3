...
Include this AFTER qgl.h in every renderer .cpp that uses qgl* calls.
For desktop GL builds the entire file is a no-op.

Design notes
────────────
Desktop-only GL enums are defined with their REAL GL numeric values,
NOT as aliases to GLES2 tokens.  Aliasing would collapse multiple
distinct desktop enums onto one GLES2 value, causing duplicate case
labels in switch statements (compile error).

Actual glTexImage2D / glCopyTexImage2D calls must convert the desktop
internalFormat to a GLES2-valid one.  Use GLES2_INTERNAL_FMT() for
that.  Informational switch statements (BitsForInternalFormat, Print)
can use the desktop enum values directly – they never reach the driver.
===========================================================================
*/
...
#define R_FindARBProgram(target, program) 0
...
// ---------------------------------------------------------------------------
// Texture-target enable/disable helpers
//
// GLES2 does not support glEnable/glDisable for texture targets.
// Textures are activated solely by glBindTexture.
// These helpers replace the qglEnable/qglDisable calls in Bind() and
// BindNull().  See Image_load.patch for usage.
// ---------------------------------------------------------------------------
static inline void gles2_tex_disable(GLenum target)
{
    // Unbind the target so it is no longer active
    GLenum t = (target == (GLenum)GL_TEXTURE_CUBE_MAP_EXT)
             ? GL_TEXTURE_CUBE_MAP : target;
    if (t != (GLenum)0x806F /* GL_TEXTURE_3D — not core GLES2 */)
        glBindTexture(t, 0);
}
static inline void gles2_tex_enable(GLenum) { /* the following glBindTexture handles this */ }
